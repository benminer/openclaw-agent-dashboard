...
export function StatsBar({ backups }: Props) {
  ...
  const latest = backups.length
    ? backups.reduce((a, b) => (new Date(a.lastModified) > new Date(b.lastModified) ? a : b))
    : null
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {stats.map((s) => (
        <div key={s.label} className="bg-gray-900/80 border border-gray-800/60 rounded-xl px-4 py-3.5">
          <p className="text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1">{s.label}</p>
          <p className={`text-xl font-semibold font-mono ${s.accent}`}>{s.value}</p>
        </div>
      ))}
    </div>
  )
